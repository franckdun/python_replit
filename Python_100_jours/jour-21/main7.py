...
def jeu_tables_multiplication():
    ...
    choix_table = input(
        "Voulez-vous réviser toutes les tables (T) ou une table spécifique (S) ? "
    ).upper()
    ...
    if choix_table == 'T':
        ...
        table = int(
            input(
                "Choisissez la table de multiplication que vous voulez réviser de (1-10) : "
            ))

        # Vérifie si la table choisie est valide
        if not 1 <= table <= 10:
            print(
                "Veuillez choisir une table de multiplication entre 1 et 10.")
            return

        tables = [table]
    else:
        print(
            "Veuillez choisir 'T' pour toutes les tables ou 'S' pour une table spécifique."
        )
        return

    # Demande à l'utilisateur de choisir le niveau de difficulté
    niveau_difficulte = int(
        input("Choisissez le niveau de difficulté de (1-3) : "))
    ...
    victoire_requise = niveaux_difficulte[niveau_difficulte][
        'victoire_requise']

    print(
        f"\nSuper ! Vous avez choisi le niveau {niveau_difficulte} de difficulté."
    )
    #affiche le nombre de victoire requis pour gagner
    print(f"Vous devez obtenir {victoire_requise} bonne réponses pour gagné.")
    print("A chaque erreur vous perdrez 1 point")
    # affiche le nombre d'erreurs possibles
    print(f"Cependant, vous avez {erreurs_restantes} chances erreurs,")
    ...
    for i in range(2, 11):
        ...
        while erreurs_restantes > 0:
            ...
            user_input = input(
                f"{i -1}. Combien font {multiplicateur} x {table} ? ")
            ...
            if reponse == multiplicateur * table:
                ...
                print(
                    f"Dommage. La réponse correcte était {multiplicateur} x {table} = {multiplicateur * table}."
                )
                erreurs_restantes -= 1
        ...
        print(
            f"Votre score actuel est de {score} bonnes réponses sur {i -1}.\n")

        # Vérifie si le joueur a atteint le nombre de bonnes réponses requis pour la victoire
        if score == victoire_requise:
            print(
                "Félicitations ! Vous avez atteint le nombre requis de bonnes réponses. Vous avez gagné !"
            )
