...
def display_dice_and_score(dice):
    ...
    score = sum(dice)
    print("Nombre obtenu -> ", convert_dice_to_number(dice), "\n")
...
def play_421():
    ...
    print("Bienvenue !\nPour gagner obtiens 421 avec les dés\nou fais le plus de points possible.")
    print("Faire 'q' pour quitter le jeu.\n")

    while True:
        num_turns += 1
        print(f"< NOUVEAU TOUR N°{num_turns} >\n")
        ...
        choices = input("Quels dés veux-tu relancer ?\n(sépare tes choix par un espace)\nFaire 'all' pour tous les dés \nFaire 'q' pour quiter :").split()
